...
# Obtain the string from a image file:
file_name = ''  # write here the path (if need) and full file name
try:
...
# Convert binary encode into string encode:
photo_s = pic_b.decode('utf-8')

# Create image file from string:
image_result = open('test_file.png', 'wb')  # create file
image_result.write(base64.decodebytes(pic_b))  # write file for binary encode
image_result.write(base64.decodebytes(pic_s.encode()))  # write file for string encode

# Show an image view:
pic_bytes = base64.b64decode(pic_b)  # trasform into bytes with Base64
